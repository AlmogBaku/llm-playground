    ...
    return <div className="rating rating-xs rating-half">
        ...
        {range(5).map((i) => (
            <Fragment key={i}><input type="radio"
                                     className={`mask mask-star-2 mask-half-1`} onChange={() => onChange(i)}
                                     checked={value ? value == i : false}/>
                <input type="radio"
                       className={`mask mask-star-2 mask-half-2`} onChange={() => onChange(i + 0.5)}
                       checked={value ? value + 0.5 == i : false}/></Fragment>
        ))}
    </div>
}